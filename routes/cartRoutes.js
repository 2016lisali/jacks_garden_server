...
import { validate, createCartSchema, createCartDetailsSchema } from '../middlewares/dataValidator.js';

const router = express.Router()
router.get("/search", verifyTokenAndAuthentication, insertLog, getCartDetailsByUserId);
router.post("/details", verifyTokenAndAuthentication, insertLog, validate(checkSchema(createCartDetailsSchema)), addCartDetails)
router.post("/", verifyToken, insertLog, validate(checkSchema(createCartSchema)), createCart);
router.patch("/:id", verifyTokenAndAuthentication, insertLog, validate(checkSchema(createCartDetailsSchema)), updateCartDetails)
