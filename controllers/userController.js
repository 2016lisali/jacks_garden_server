...
export const createUser = async (req, res) => {
    ...
    try {
        ...
        res.status(201).json(
            `user created with id ${data.insertId}, cart ${cart.insertId}`
        );
    } catch (error) {
        console.log(error);
        return res
            .status(500)
            .json("userController error. query error - failed to create user");
    }
};
...
export const updateUser = async (req, res) => {
    ...
    !user.password.startsWith("$2b$") &&
        (hashedPassword = bcrypt.hashSync(user.password, 12));
    try {
...
export const login = async (req, res) => {
    ...
    try {
        ...
        if (!isPasswordCorrect)
            return res
                .status(400)
                .json("Password is not match with the account.");
