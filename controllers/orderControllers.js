...
export const createOrderBillingDetails = async (req, res) => {
    ...
    if (details.userId != req.user.userId)
        return res.status(403).json("You are not authorized");
    try {
        const result = await orderModel.createOrderBillingDetails(
            details.orderId,
            details.name,
            details.email,
            details.phone,
...
export const getOrderDetails = async (req, res) => {
    try {
        ...
        if (
            result.length > 0 &&
            (result[0].userId == req.user.userId || req.user.isAdmin == 1)
        ) {
...
export const getOrdersByUserId = async (req, res) => {
    if (req.params.id != req.user.userId && req.user.isAdmin != 1)
        return res.status(403).json("You are not authorized");
    try {
...
export const getOrderBillingDetails = async (req, res) => {
    try {
        const result = await orderModel.getOrderBillingDetailsByOrderId(
            req.params.id
        );
        result.length > 0
            ? res.status(200).json(result)
